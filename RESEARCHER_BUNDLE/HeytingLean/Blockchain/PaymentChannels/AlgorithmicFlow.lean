...
private instance : Std.Antisymm (sym2Le (V := V)) :=
  ⟨by
    intro a b hab hba
...
private def Indexing.edgeCount {G : ChannelGraph V} (idx : Indexing (V := V) G) : Nat := idx.edges.length
private def Indexing.vertCount {G : ChannelGraph V} (idx : Indexing (V := V) G) : Nat := idx.verts.length
private def Indexing.edgeBase {G : ChannelGraph V} (_idx : Indexing (V := V) G) : Nat := 1
private def Indexing.vertBase {G : ChannelGraph V} (idx : Indexing (V := V) G) : Nat := idx.edgeBase + idx.edgeCount
private def Indexing.sinkIdx {G : ChannelGraph V} (idx : Indexing (V := V) G) : Nat := idx.vertBase + idx.vertCount
private def Indexing.nodeCount {G : ChannelGraph V} (idx : Indexing (V := V) G) : Nat := idx.sinkIdx + 1
private def Indexing.edgeNodeIdx {G : ChannelGraph V} (idx : Indexing (V := V) G) (ei : Nat) : Nat := idx.edgeBase + ei
private def Indexing.vertNodeIdx {G : ChannelGraph V} (idx : Indexing (V := V) G) (vi : Nat) : Nat := idx.vertBase + vi
